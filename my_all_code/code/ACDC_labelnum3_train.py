...
from networks.net_factory_ega import BCP_net, net_factory
from utils import losses, ramps, feature_memory, contrastive_losses, val_2d

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='/workspace/my_all_code/data_split/ACDC', help='Name of Experiment')
...
parser.add_argument('--run_mode', type=str, choices=['pretrain', 'selftrain', 'pretrain_then_selftrain'], required=True, help='whether to perform pre-training, self-training, pre-training then self-training')
...
def generate_dynamic_mask(img, iter_num, max_iterations):
    batch_size, channel, img_x, img_y = img.shape[0], img.shape[1], img.shape[2], img.shape[3]
    loss_mask = torch.ones(batch_size, img_x, img_y).cuda()
    mask = torch.ones(img_x, img_y).cuda()

    # 动态调整遮罩大小，从2/3线性减少到1/3
    scale_factor = 2/3 - (iter_num / max_iterations) * (2/3 - 1/3)
    patch_x, patch_y = int(img_x * scale_factor), int(img_y * scale_factor)
...
def pre_train(args, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            img_mask, loss_mask = generate_dynamic_mask(img_a, iter_num, max_iterations)
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    model = BCP_net(in_chns=1, class_num=num_classes)
    ####################################################################
    static_teacher_model = BCP_net(in_chns=1, class_num=num_classes, ema=True)
    dynamic_teacher_model = BCP_net(in_chns=1, class_num=num_classes, ema=True)
    ###################################################################
    ...
    optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=0.9, weight_decay=0.0001)
    #########################################################
    load_net(static_teacher_model, pre_trained_model)
    load_net(dynamic_teacher_model, pre_trained_model)
    #########################################################
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                ############################################
                static_teacher_model_pre_a = static_teacher_model(uimg_a)
                dynamic_teacher_model_pre_a = dynamic_teacher_model(uimg_a)
                static_teacher_model_pre_b = static_teacher_model(uimg_b)
                dynamic_teacher_model_pre_b = dynamic_teacher_model(uimg_b)
            ...
            ######################################
            iter_num += 1
            update_model_ema(model, dynamic_teacher_model, 0.89)
            ######################################
            ...
            if update_counter >= update_threshold:
                static_teacher_model.load_state_dict(model.state_dict())
...
def main(args):
    ...
    shutil.copy('../code/ACDC_labelnum3_train.py', self_snapshot_path)
    ...
    if args.run_mode == 'pretrain':
        ...
        print("...",pre_snapshot_path)
        model, optimizer = pre_train(args, pre_snapshot_path)
        save_net_opt(model, optimizer, os.path.join(pre_snapshot_path, 'final_pretrain_model.pth'))

    if args.run_mode == 'selftrain':
    ...
    if args.run_mode == 'pretrain_then_selftrain':
        logging.basicConfig(filename=pre_snapshot_path + "/log.txt", level=logging.INFO,
                            format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
        logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
        logging.info(str(args))
        model, optimizer = pre_train(args, pre_snapshot_path)
        save_net_opt(model, optimizer, os.path.join(pre_snapshot_path, 'final_pretrain_model.pth'))
        print("Pre-training completed. Starting self-training...")
        command = f"python {sys.argv[0]} --run_mode selftrain"
        os.system(command)
...
### python ACDC_labelnum3_train.py --gpu 0 --run_mode pretrain_then_selftrain