...
from networks.net_factory_ega import net_factory
...
def Inference(FLAGS):
    ...
    for case in tqdm(image_list):
        ...
        overall_avg_metric = (np.asarray(first_metric) + np.asarray(second_metric) + np.asarray(third_metric)) / 3
        print(f"Case: {case}")
        print(f"First Metric: {first_metric}")
        print(f"Second Metric: {second_metric}")
        print(f"Third Metric: {third_metric}")
        print(f"Average Metrics: {overall_avg_metric}")
        print("-" * 30)
...
# python test_ACDC.py --labelnum 3
# python test_ACDC.py --labelnum 7
