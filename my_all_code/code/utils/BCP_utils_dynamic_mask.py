...
def context_dynamic_mask(img, iter_num, max_iterations):
    batch_size, channel, img_x, img_y, img_z = img.shape
    loss_mask = torch.ones(batch_size, img_x, img_y, img_z).cuda()
    mask = torch.ones(img_x, img_y, img_z).cuda()

    # 动态调整遮罩大小，从2/3线性减少到1/3
    #scale_factor = 2/3 - (iter_num / max_iterations) * (1/3)
    final_scale = 1/2
    scale_factor = 2/3 - (iter_num / max_iterations) * (2/3 - final_scale)
    patch_pixel_x, patch_pixel_y, patch_pixel_z = int(img_x * scale_factor), int(img_y * scale_factor), int(img_z * scale_factor)
