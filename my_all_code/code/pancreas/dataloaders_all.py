...
def get_dataset_path(dataset='pancreas', labelp='10percent'):
    ...
    return ['/'.join(['/workspace/my_all_code/code/pancreas/data_lists', dataset,  f]) for f in files]
...
def get_ema_model_and_dataloader(data_root, split_name, batch_size, lr, labelp=10):
    ...
    """Net & optimizer"""
    net = create_Vnet()
    static_teacher_net = create_Vnet(ema=True).cuda()
    dynamic_teacher_net = create_Vnet(ema=True).cuda()
    ...
    return net, static_teacher_net, dynamic_teacher_net, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader