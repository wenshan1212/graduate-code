...
def generate_mask_dynamic(img, iter_num, max_iterations):
    batch_l = img.shape[0]
    base_patch_size = 96
    loss_mask = torch.ones(batch_l, base_patch_size, base_patch_size, base_patch_size).cuda()
    mask = torch.ones(base_patch_size, base_patch_size, base_patch_size).cuda()
    
    # 计算当前的缩放因子，动态调整遮罩大小从2/3到1/3
    final_scale = 5/9  # 或者其他你认为更合适的值
    scale_factor = 2/3 - (iter_num / max_iterations) * (2/3 - final_scale)
    
    # 计算遮罩的实际大小，考虑到初始遮罩大小为64×64×64
    #actual_patch_size = int(64 * scale_factor)
    # up maybe is wrong
    actual_patch_size = int(96 * scale_factor)
    
    # 确保随机选取的起始点允许完整的遮罩放置在图像内
    w = np.random.randint(0, base_patch_size - actual_patch_size)
    h = np.random.randint(0, base_patch_size - actual_patch_size)
    z = np.random.randint(0, base_patch_size - actual_patch_size)
    
    # 应用遮罩
    mask[w:w+actual_patch_size, h:h+actual_patch_size, z:z+actual_patch_size] = 0
    loss_mask[:, w:w+actual_patch_size, h:h+actual_patch_size, z:z+actual_patch_size] = 0
