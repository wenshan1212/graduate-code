...
def pretrain(net1, optimizer, lab_loader_a, labe_loader_b, test_loader):
    ...
    for epoch in tqdm_load(range(1, pretraining_epochs + 1), ncols=70):
        ...
        if epoch % pretrain_save_step == 0:
            avg_metric1, metric_list = test_calculate_metric(net1, test_loader.dataset)
        ...
        for step, ((img_a, lab_a), (img_b, lab_b)) in enumerate(zip(lab_loader_a, lab_loader_b)):
            ...
            img_mask, loss_mask = generate_mask_dynamic(img_a, epoch, pretraining_epochs)   
            ...
            loss = 0.45 * ce_loss1 + 0.55 * dice_loss1
...
def ema_cutmix(net, static_teacher_net, dynamic_teacher_net, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader):
    """Create Path"""
    ...
    load_net_opt(static_teacher_net, optimizer, pretrained_path / f'best_ema{label_percent}_pre.pth')
    load_net_opt(dynamic_teacher_net, optimizer, pretrained_path / f'best_ema{label_percent}_pre.pth')
    logger.info('Loaded from {}'.format(pretrained_path))
    update_counter = 0  # 用于控制更新时机的计数器
    update_threshold = 10  # 初始更新阈值
    max_dice = 0
    max_list = None
    for epoch in tqdm_load(range(1, self_training_epochs+1)):
        ...
        if epoch % st_save_step == 0:
            avg_metric, metric_list = test_calculate_metric(net, test_loader.dataset)
            ...
            if val_dice > max_dice:
                ...
                max_dice = val_dice
                max_list = avg_metric
                update_threshold = min(update_threshold + 5, 20)  # 提升性能，增加更新阈值
            else:
                update_threshold = max(update_threshold - 5, 5)  # 性能下降，减少更新阈值
        ...
        if update_counter >= update_threshold:
            static_teacher_net.load_state_dict(net.state_dict())
            update_counter = 0  # 重置计数器

        """Training"""
        net.train()
        static_teacher_net.train()
        dynamic_teacher_net.train()
        ...
        for step, ((img_a, lab_a), (img_b, lab_b), (unimg_a, unlab_a), (unimg_b, unlab_b)) in enumerate(zip(lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b)):
            ...
            with torch.no_grad():
                ...
                static_teacher_net_unimg_a_out = static_teacher_net(unimg_a)[0]
                static_teacher_net_unimg_b_out = static_teacher_net(unimg_b)[0]
                dynamic_teacher_net_unimg_a_out = dynamic_teacher_net(unimg_a)[0]
                dynamic_teacher_net_unimg_b_out = dynamic_teacher_net(unimg_b)[0]
                unimg_a_out = (static_teacher_net_unimg_a_out + dynamic_teacher_net_unimg_a_out) / 2
                unimg_b_out = (static_teacher_net_unimg_b_out + dynamic_teacher_net_unimg_b_out) / 2
                ...
                #img_mask, loss_mask = generate_mask(img_a, patch_size)  
                img_mask, loss_mask = generate_mask_dynamic(img_a, epoch, self_training_epochs)        
            ...
            update_ema_variables(net, dynamic_teacher_net, alpha)
...
if __name__ == '__main__':
    try:
        ...
        ema_cutmix(net, static_teacher_net, dynamic_teacher_net, optimizer, lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b, test_loader)
