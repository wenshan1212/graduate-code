...
from networks.net_factory_la_ega import net_factory
from utils.BCP_utils_dynamic_mask import context_mask, context_dynamic_mask, mix_loss, parameter_sharing, update_ema_variables

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='/workspace/my_all_code/data_split/LA/', help='Name of Dataset')
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                img_mask, loss_mask = context_dynamic_mask(img_a, iter_num, pre_max_iterations)
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    s_model = net_factory(net_type=args.model, in_chns=1, class_num=num_classes, mode="train")
    static_teacher_model = net_factory(net_type=args.model, in_chns=1, class_num=num_classes, mode="train")
    dynamic_teacher_model = net_factory(net_type=args.model, in_chns=1, class_num=num_classes, mode="train")
    s_model = nn.DataParallel(s_model)
    s_model = s_model.cuda()
    static_teacher_model = nn.DataParallel(static_teacher_model)
    static_teacher_model = static_teacher_model.cuda()
    dynamic_teacher_model = nn.DataParallel(dynamic_teacher_model)
    dynamic_teacher_model = dynamic_teacher_model.cuda()
    

    for param in static_teacher_model.parameters():
            param.detach_()   # static_teacher_model set
    for param in dynamic_teacher_model.parameters():
            param.detach_()   # dynamic_teacher_model set
    db_train = LAHeart(base_dir=train_data_path,
                       split='train',
                       transform = transforms.Compose([
                          RandomRotFlip(),
    ...
    load_net(s_model, pretrained_model)
    load_net(static_teacher_model, pretrained_model)
    load_net(dynamic_teacher_model, pretrained_model)

    
    s_model.train()
    #ema_model.train()
    static_teacher_model.train()
    dynamic_teacher_model.train()
    update_counter = 0  # 用于控制更新时机的计数器
    update_threshold = 1000  # 初始更新阈值
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                static_teacher_model_pre_a, _ = static_teacher_model(unimg_a)
                dynamic_teacher_model_pre_a, _ = dynamic_teacher_model(unimg_a)
                static_teacher_model_pre_b, _ = static_teacher_model(unimg_b)
                dynamic_teacher_model_pre_b, _ = dynamic_teacher_model(unimg_b)
                pre_a = (static_teacher_model_pre_a + dynamic_teacher_model_pre_a) / 2
                pre_b = (static_teacher_model_pre_b + dynamic_teacher_model_pre_b) / 2
                
                
                plab_a = get_cut_mask(pre_a, nms=1)
                plab_b = get_cut_mask(pre_b, nms=1)
                img_mask, loss_mask = context_dynamic_mask(img_a, iter_num, self_max_iterations)
            ...
            outputs_l, _ = s_model(mixl_img)
            outputs_u, _ = s_model(mixu_img)
            ...
            ##################################################
            update_ema_variables(s_model, dynamic_teacher_model, 0.99)
            ##################################################
            ...
            if iter_num % 200 == 0:
                s_model.eval()
                dice_sample = test_3d_patch.var_all_case_LA(s_model, num_classes=num_classes, patch_size=patch_size, stride_xy=18, stride_z=4)
                
                ##########################################################################
                if dice_sample > best_dice:
                    best_dice = dice_sample
                    update_threshold = min(update_threshold + 500, 2000)  # 提升性能，增加更新阈值
                    best_dice = round(dice_sample, 4)
                    ...
                    update_threshold = max(update_threshold - 500, 500)  # 性能下降，减少更新阈值
                ##############################################################################
                writer.add_scalar('4_Var_dice/Dice', dice_sample, iter_num)
                writer.add_scalar('4_Var_dice/Best_dice', best_dice, iter_num)
                s_model.train()
            ...
            if update_counter >= update_threshold:
                static_teacher_model.load_state_dict(s_model.state_dict())
...
if __name__ == "__main__":
    ...
    shutil.copy('../code/LA_labelnum8_train.py', self_snapshot_path)
    # -- Pre-Training
